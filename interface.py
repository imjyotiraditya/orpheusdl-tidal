...
from .mqa_identifier_python.mqa_identifier_python.mqa_identifier import (
    MqaIdentifier,
)
...
class ModuleInterface:
        ...
        while True:
            ...
            def auth_and_save_session(session, session_type):
                session = self.auth_session(
                    session, session_type, login_session
                )
            ...
            if not saved_sessions:
                ...
                if len(self.available_sessions) == 1:
                    ...
                    self.print(
                        f'{module_information.service_name}: 1. TV (browser)'
                    )
                    self.print(
            ...
            for session_type in self.available_sessions:
                ...
                if session_type in saved_sessions:
                    ...
                    sessions[session_type].set_storage(
                        saved_sessions[session_type]
                    )
                else:
                    logging.debug(
                ...
                if not sessions[session_type].valid():
                    ...
                    saved_sessions[session_type] = sessions[
                        session_type
                    ].get_storage()
                    module_controller.temporary_settings_controller.set(
                        'sessions', saved_sessions
    ...
    def init_session(self, session_type):
        ...
        if session_type == SessionType.TV.name:
            session = TidalTvSession(
                self.settings['tv_atmos_token'],
                self.settings['tv_atmos_secret'],
            )
        elif session_type == SessionType.MOBILE_ATMOS.name:
            session = TidalMobileSession(
                self.settings['mobile_atmos_hires_token']
            )
        else:
    ...
    def auth_session(self, session, session_type, login_session):
        if login_session:
            ...
            self.print(
                f'{module_information.service_name}: Creating a TV session'
            )
            session.auth()
        else:
            self.print(
                f'{module_information.service_name}: Creating a Mobile session'
            )
            self.print(
    ...
    def get_artist_info(
        ...
        artist_singles = self.session.get_artist_albums_ep_singles(
            artist_id
        ).get('items')
        ...
        if (
            ...
            # This is so retarded
            page_list = credited_albums_page['rows'][-1]['modules'][0].get(
                'pagedList'
            )
            if page_list:
                ...
                credit_albums = [
                    item.get('item').get('album') for item in items
                ]
    ...
    def get_album_info(self, album_id: str, data=None) -> AlbumInfo:
        ...
        cache = {'data': {}}
        try:
            tracks_data = self.session.get_album_contributors(
                album_id, limit=limit
            )
            ...
            for offset in range(
                limit, ((total_tracks // limit) + 1) * limit, limit
            ):
            ...
            for track in tracks_data.get('items'):
                ...
                cache.get('data')[str(track.get('item').get('id'))] = track.get(
                    'item'
                )
        ...
        if album_data.get('releaseDate'):
            ...
            release_year = [
                int(s)
                for s in album_data.get('copyright').split()
                if s.isdigit()
            ]
    ...
    def get_track_info(
        ...
        track_data = (
            data[track_id]
            if track_id in data
            else self.session.get_track(track_id)
        )
        ...
        try:
            album_data = (
                data[album_id]
                if album_id in data
                else self.session.get_album(album_id)
            )
        ...
        if stream_data is not None:
            ...
            if audio_track:
                ...
                if track_codec is CodecEnum.MQA and self.settings['fix_mqa']:
                    ...
                    temp_file_path = self.download_temp_header(
                        manifest['urls'][0]
                    )
        ...
        bit_depth = (
            (
                24
                if stream_data
                and stream_data['audioQuality'] == 'HI_RES_LOSSLESS'
                else 16
            )
        ...
        track_info = TrackInfo(
            ...
            else track_data.get('dateAdded')[:4]
            ...
            for adaptation_set in period.findall('AdaptationSet'):
                for rep in adaptation_set.findall('Representation'):
                    ...
                    if seg_timeline is not None:
                        ...
                        seg_num_list = list(
                            range(
                                start_number, len(seg_time_list) + start_number
                            )
                        )
                        # Replace $Number$ with all the seg_num_list indices
                        track_urls += [
                            seg_template.get('media').replace(
                                '$Number$', str(n)
                            )
    ...
    def get_track_download(
        ...
        if file_url:
            return TrackDownloadInfo(
                download_type=DownloadEnum.URL, file_url=file_url
            )
        ...
        for download_url in bar:
            temp_locations.append(
                download_to_temp(download_url, extension='mp4')
            )
        ...
        output_location = (
            create_temp_filename()
            + '.'
            + codec_data[audio_track.codec].container.name
        )
    ...
    def get_track_cover(
        ...
        if cover_id:
            return CoverInfo(
                url=self._generate_artwork_url(
    ...
    def get_track_lyrics(
        self, track_id: str, track_data: dict = None
    ) -> LyricsInfo:
        if not track_data:
            track_data = {}
        ...
        if 'error' in lyrics_data and track_data:
            ...
            lyrics_data = (
                self.session.get_lyrics(best_tracks[0])
    ...
    def get_track_credits(self, track_id: str, data=None) -> Optional[list]:
        ...
        if track_id in data:
            ...
            track_contributors = self.session.get_track_contributors(
                track_id
            ).get('items')
        ...
        if len(credits_dict) > 0:
            ...
            return [
                CreditsInfo(sanitise_name(k), v)
                for k, v in credits_dict.items()
            ]
        return None
